...
function printHelp() {
  echo "Usage: "
  echo "  runApp.sh <mode> [-c <channel name>] [-t <timeout>] [-d <delay>] [-f <docker-compose-file>] [-s <dbtype>] [-l <language>] [-o <consensus-type>] [-i <imagetag>] [-v]"
  ...
  echo "    -v - verbose mode"
  echo "  runApp.sh -h (print this message)"
  echo
  echo "Typically, one would first generate the required certificates and "
  echo "genesis block, then bring up the network. e.g.:"
  echo
  echo "	runApp.sh generate -c mychannel"
  echo "	runApp.sh up -c mychannel -s couchdb"
  echo "        runApp.sh up -c mychannel -s couchdb -i 1.4.0"
  echo "	runApp.sh up -l node"
  echo "	runApp.sh down -c mychannel"
  echo "        runApp.sh upgrade -c mychannel"
  echo
  echo "Taking all defaults:"
  echo "	runApp.sh generate"
  echo "	runApp.sh up"
  echo "	runApp.sh down"
}
...
function upgradeNetwork() {
  if [[ "$IMAGETAG" == *"1.4"* ]] || [[ $IMAGETAG == "latest" ]]; then
	...
	#removing containers
	docker-compose -f $COMPOSE_FILES down -d
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    rm -rf artifacts/channel/*.block artifacts/channel/*.tx crypto-config ./org3-artifacts/crypto-config/ artifacts/channel/org3.json
...
# artifacts, and subsequently output these files into the ``artifacts/channel``
# folder.
#
...
function generateChannelArtifacts() {
  ...
  if [ "$CONSENSUS_TYPE" == "solo" ]; then
    configtxgen -profile TwoOrgsOrdererGenesis -channelID byfn-sys-channel -outputBlock ./artifacts/channel/genesis.block
  elif [ "$CONSENSUS_TYPE" == "kafka" ]; then
    configtxgen -profile SampleDevModeKafka -channelID byfn-sys-channel -outputBlock ./artifacts/channel/genesis.block
  elif [ "$CONSENSUS_TYPE" == "etcdraft" ]; then
    configtxgen -profile SampleMultiNodeEtcdRaft -channelID byfn-sys-channel -outputBlock ./artifacts/channel/genesis.block
  else
    set +x
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile TwoOrgsChannel -outputCreateChannelTx ./artifacts/channel/channel.tx -channelID $CHANNEL_NAME
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  configtxgen -profile TwoOrgsChannel -outputAnchorPeersUpdate \
    ./artifacts/channel/Org2MSPanchors.tx -channelID $CHANNEL_NAME -asOrg Org2MSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
